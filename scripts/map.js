...
myFunctionHolder.pointToEllipse = function (row) {
    ...
    const lng = parseFloat(row.LON_ELLI_IMG) - 180;
...
window.onload = function () { // window.onload allows for the map to load after the page has loaded
    let mapObject = L.map('map').setView([0, 0], 2);  // global view
    let baseMap = L.tileLayer('http://s3-eu-west-1.amazonaws.com/whereonmars.cartodb.net/viking_mdim21_global/{z}/{x}/{y}.png', {
        maxZoom: 18,
        attribution: '&copy; <a href=”https://www.openplanetary.org/opm-basemaps/global-viking-mdim2-1-colorized-mosaic”>OpenPlanetary</a>'
    });
